...
class TwoFactorAuthController extends Controller
{
    ...
    public function showVerify()
    {
        ...
        // CORREÇÃO: Usar o método do modelo User para email
        if ($user->two_factor_method === 'email') {
            try {
                $result = $user->sendTwoFactorEmailCode();
                
                if (!$result) {
                    Log::error('Falha ao enviar código 2FA por email', [
                        'user_id' => $user->id,
                        'email' => $user->email
                    ]);
                ...
                Log::info('Código 2FA enviado com sucesso', [
                    'user_id' => $user->id,
                    'method' => 'email'
                ]);
                
            } catch (\Exception $e) {
                Log::error('Exceção ao enviar código 2FA por email', [
                    'user_id' => $user->id,
                    'error' => $e->getMessage(),
    ...
    public function resendEmailCode(Request $request)
    {
        ...
        try {
            ...
            if (!$result) {
                Log::error('Falha ao reenviar código 2FA por email', [
                    'user_id' => $user->id,
                    'email' => $user->email
                ]);
            ...
            Log::info('Código 2FA reenviado com sucesso', [
                'user_id' => $user->id,
                'ip' => $request->ip()
            ]);
     ...
     */
    private function verifyEmailCode($user, $code)
    {
        if (!$user->two_factor_email_code || !$user->two_factor_email_code_expires_at) {
            return false;
        }
        
        if (now()->isAfter($user->two_factor_email_code_expires_at)) {
            return false;
        }
        
        return $user->two_factor_email_code === $code;
    }
