...
return new class extends Migration
{
    ...
    {
        Schema::create('fal7_mercadorias_perigosas', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->uuid('pedido_id');
            
            // Informações gerais
            $table->boolean('possui_mercadorias_perigosas')->default(false);
            $table->text('observacoes_gerais')->nullable();
            $table->text('medidas_seguranca')->nullable();
            $table->string('responsavel_tecnico')->nullable();
            $table->string('certificacao_responsavel')->nullable();
