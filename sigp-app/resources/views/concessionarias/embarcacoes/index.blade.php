<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('concessionarias.embarcacoes.create') }}" class="text-white font-bold py-1.5 px-4 rounded-lg transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center text-xs" style="background: #3395da;">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
          ...
          <!-- Seção de Mensagens de Erro e Sucesso -->
            ...
            <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div class="p-4">
                    <form method="GET" action="{{ route('concessionarias.embarcacoes.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <div>
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-3 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('concessionarias.embarcacoes.relatorio-filtrado', array_merge(request()->query(), ['formato' => 'pdf'])) }}" 
                            ...
                            <a href="{{ route('concessionarias.embarcacoes.relatorio-filtrado', array_merge(request()->query(), ['formato' => 'excel'])) }}" 
                               class="text-white px-3 py-1.5 rounded-md text-xs font-medium transition duration-200 flex items-center hover:shadow-lg transform hover:-translate-y-0.5" 
                               style="background: #059669;" target="_blank">
                                <i class="fas fa-file-excel mr-1 text-xs"></i>Relatório Excel
                            </a>
                            <a href="{{ route('concessionarias.embarcacoes.create') }}" class="text-white px-3 py-1.5 rounded-md text-xs font-medium transition duration-200 flex items-center hover:shadow-lg transform hover:-translate-y-0.5" style="background: #3395da;">
                                ...
                                <tr class="hover:bg-blue-50 transition-colors duration-200 group">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right">
                                        <div class="flex justify-end space-x-1">
                                            ...
                                            <a href="{{ route('concessionarias.embarcacoes.edit', $embarcacao) }}" 
                                               class="inline-flex items-center justify-center w-7 h-7 text-white rounded-md transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5" 
                                               style="background: #3395da;"
                                               title="Editar">
                                             ...
                                             @role('admin')  
                                                                                       <form action="{{ route('concessionarias.embarcacoes.destroy', $embarcacao) }}" 
                                                  method="POST" 
                                                  style="display: inline;"
                                                  onsubmit="return confirm('Tem certeza que deseja excluir a embarcação {{ addslashes($embarcacao->nome) }}? Esta ação não pode ser desfeita.')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        class="inline-flex items-center justify-center w-7 h-7 text-white rounded-md transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5" 
                                ...
                                <tr>
                                    <td colspan="6" class="px-4 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <a href="{{ route('concessionarias.embarcacoes.create') }}" class="text-white font-semibold py-2 px-4 rounded-md transition duration-200 flex items-center text-xs" style="background: #3395da;">
    ...
    <!-- Modal removido - exclusão direta -->
    ...
    <script>
        function confirmDelete(id, nome) {
            console.log('Abrindo modal para embarcação:', nome, 'ID:', id); // Debug
            
            // Definir o nome da embarcação no modal
            const embarcacaoNomeElement = document.getElementById('embarcacaoNome');
            if (embarcacaoNomeElement) {
                embarcacaoNomeElement.textContent = nome;
            }
            
            // Definir a ação do formulário
            const deleteForm = document.getElementById('deleteForm');
            if (deleteForm) {
                deleteForm.action = `/concessionaria/embarcacoes/${id}`;
            }
            ...
            if (modal) {
                ...
                // Forçar o foco no modal para melhor acessibilidade
                modal.focus();
            } else {
    ...
    @endpush
        ...
        <script>
            function exportData(format) {
                ...
                window.location.href = `{{ route('concessionarias.embarcacoes.index') }}?${params.toString()}`;
            }
