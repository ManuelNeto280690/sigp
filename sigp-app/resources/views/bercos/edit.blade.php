    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 text-xs mt-1">Atualize as informações do berço</p>
            </div>
            <div class="flex items-center space-x-3">
                <a href="{{ route('bercos.show', $berco) }}" class="text-white font-medium py-1.5 px-3 rounded-lg transition duration-200 text-xs" style="background: #3395da;">
                    <i class="fas fa-eye mr-1 text-xs"></i>Visualizar
                </a>
                <a href="{{ route('bercos.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-1.5 px-3 rounded-lg transition duration-200 text-xs">
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('bercos.update', $berco) }}" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select name="terminal_id" id="terminal_id" required
                                ...
                                @foreach($terminais as $terminal)
                                    <option value="{{ $terminal->id }}" {{ old('terminal_id', $berco->terminal_id) == $terminal->id ? 'selected' : '' }}>
                                        {{ $terminal->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="nome" id="nome" value="{{ old('nome', $berco->nome) }}" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:border-transparent"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="calado_maximo" id="calado_maximo" value="{{ old('calado_maximo', $berco->calado_maximo) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:border-transparent"
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="comprimento_maximo" id="comprimento_maximo" value="{{ old('comprimento_maximo', $berco->comprimento_maximo) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:border-transparent"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select name="status" id="status" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:border-transparent"
                                    style="focus:ring-color: #3b82f6;">
                                <option value="disponivel" {{ old('status', $berco->status) == 'disponivel' ? 'selected' : '' }}>Disponível</option>
                                <option value="ocupado" {{ old('status', $berco->status) == 'ocupado' ? 'selected' : '' }}>Ocupado</option>
                                <option value="manutencao" {{ old('status', $berco->status) == 'manutencao' ? 'selected' : '' }}>Manutenção</option>
                            </select>
                        </div>
                    </div>

                    <div class="flex items-center justify-end space-x-3">
                        <a href="{{ route('bercos.show', $berco) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2 px-4 rounded-lg transition duration-200 text-sm">
                            Cancelar
                        </a>
