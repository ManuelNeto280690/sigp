<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('entrada-saida-embarcacao.create') }}" class="text-white font-bold py-1.5 px-4 rounded-lg transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center text-xs" style="background: #0084de;">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div class="p-4">
                    <form method="GET" action="{{ route('entrada-saida-embarcacao.index') }}" class="space-y-4">
                        ...
                        <div class="flex flex-wrap gap-2 pt-2">
                            ...
                            <a href="{{ route('entrada-saida-embarcacao.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-md text-xs font-medium transition duration-200 flex items-center">
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-3 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('entrada-saida-embarcacao.relatorioFiltrado', array_merge(request()->query(), ['formato' => 'pdf'])) }}" class="text-white px-3 py-1.5 rounded-md text-xs font-medium transition duration-200 flex items-center hover:shadow-lg transform hover:-translate-y-0.5" style="background: #dc2626;">
                                <i class="fas fa-file-pdf mr-1 text-xs"></i>Relatório PDF
                            </a>
                            <a href="{{ route('entrada-saida-embarcacao.relatorioFiltrado', array_merge(request()->query(), ['formato' => 'excel'])) }}" class="text-white px-3 py-1.5 rounded-md text-xs font-medium transition duration-200 flex items-center hover:shadow-lg transform hover:-translate-y-0.5" style="background: #16a34a;">
                                <i class="fas fa-file-excel mr-1 text-xs"></i>Relatório Excel
                            </a>
                            <a href="{{ route('entrada-saida-embarcacao.create') }}" class="text-white px-3 py-1.5 rounded-md text-xs font-medium transition duration-200 flex items-center hover:shadow-lg transform hover:-translate-y-0.5" style="background: #0084de;">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('entrada-saida-embarcacao.show', $movimento) }}" class="text-blue-600 hover:text-blue-900 transition-colors duration-200" title="Visualizar">
                                                <i class="fas fa-eye text-xs"></i>
                                            </a>

                                            <a href="{{ route('entrada-saida-embarcacao.exportPdf', $movimento) }}" 
                                               class="text-red-600 hover:text-blue-900 transition-colors duration-200" 
                                               title="Baixar PDF">
                                                <i class="fas fa-file-pdf text-xs"></i>
                                            </a>
                                         
                                            @if($movimento->status !== 'concluido')
                                                <a href="{{ route('entrada-saida-embarcacao.edit', $movimento) }}" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-200" title="Editar">
                                                    <i class="fas fa-edit text-xs"></i>
                                                </a>
                                            @endif
                                           
                                            @if($movimento->status === 'programado' && Auth::user()->hasRole('admin'))
                                            ...
                                            @if(($movimento->status === 'concluido' || $movimento->status === 'cancelado') && Auth::user()->hasRole('admin'))
                                                <button type="button" 
                                                        onclick="showRevokeModal('{{ $movimento->id }}', '{{ ($movimento->embarcacao->nome ?? 'N/A') . ' - ' . $movimento->tipo_movimento }}')"
                                                        class="text-orange-600 hover:text-orange-900 transition-colors duration-200" 
                                                        title="Revogar para Edição">
                                                    <i class="fas fa-undo text-xs"></i>
                                                </button>
                                            @endif
                                        
                                            @if($movimento->status !== 'concluido' && Auth::user()->hasRole('admin'))
                                                <form action="{{ route('entrada-saida-embarcacao.destroy', $movimento) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" 
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Revogar Movimento</h3>
                ...
                <div class="mb-4">
                    <p class="text-sm text-gray-600">Você está prestes a revogar o movimento:</p>
                    <p id="revokeMovementInfo" class="font-semibold text-gray-900 mt-1"></p>
                    <p class="text-xs text-orange-600 mt-2">
                        <i class="fas fa-exclamation-triangle mr-1"></i>
                        Esta ação permitirá que o movimento seja editado novamente.
                    </p>
                </div>
                ...
                <form id="revokeForm" method="POST">
                    ...
                    <div class="flex items-center justify-end space-x-3">
                        <button type="button" onclick="closeRevokeModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200">
                            Cancelar
                        </button>
                        <button type="submit" class="px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition-colors duration-200">
                            Revogar Movimento
                        </button>
                    </div>
                </form>
    ...
    <script>
    function showRevokeModal(movimentoId, movimentoInfo) {
        document.getElementById('revokeMovementInfo').textContent = movimentoInfo;
        document.getElementById('revokeForm').action = `/entrada-saida-embarcacao/${movimentoId}/revogar`;
        document.getElementById('motivo_revogacao').value = '';
     ...
     <script>
        ...
        function confirmDelete(id, movimentoInfo) {
            ...
            if (modal && infoSpan && deleteForm) {
                ...
                deleteForm.action = `/entrada-saida-embarcacao/${id}`;
