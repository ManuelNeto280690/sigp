    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 text-xs mt-1">Edite as informações do pedido</p>
            </div>
            <div class="flex items-center space-x-3">
                <a href="{{ route('pedidos.show', $pedido) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-1.5 px-3 rounded-lg transition duration-200 flex items-center text-xs">
    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                <form action="{{ route('pedidos.update', $pedido) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="nome_navio" id="nome_navio" value="{{ old('nome_navio', $pedido->nome_navio) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                            
                            <div>
                                <label for="numero_imo" class="block text-sm font-medium text-gray-700 mb-1">Número IMO *</label>
                                <input type="text" name="numero_imo" id="numero_imo" value="{{ old('numero_imo', $pedido->numero_imo) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                            
                            <div>
                                <label for="indicativo_chamada" class="block text-sm font-medium text-gray-700 mb-1">Indicativo de Chamada *</label>
                                <input type="text" name="indicativo_chamada" id="indicativo_chamada" value="{{ old('indicativo_chamada', $pedido->indicativo_chamada) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                            
                            <div>
                                <label for="viagem_numero" class="block text-sm font-medium text-gray-700 mb-1">Número da Viagem</label>
                                <input type="text" name="viagem_numero" id="viagem_numero" value="{{ old('viagem_numero', $pedido->viagem_numero) }}" 
                            ...
                            <div>
                                ...
                                <select name="bandeira_navio" id="bandeira_navio" 
                                    ...
                                    @foreach(\App\Helpers\CountryHelper::getCountriesForSelect() as $code => $country)
                                        <option value="{{ $country }}" {{ old('bandeira_navio', $pedido->bandeira_navio) == $country ? 'selected' : '' }}>
                                            {{ $country }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="date" name="data_chegada" id="data_chegada" value="{{ old('data_chegada', $pedido->data_chegada) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" >
                            </div>
                            
                            <div>
                                <label for="hora_chegada" class="block text-sm font-medium text-gray-700 mb-1">Hora de Chegada *</label>
                                <input type="time" name="hora_chegada" id="hora_chegada" value="{{ old('hora_chegada', $pedido->hora_chegada) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" >
                            </div>
                            
                            <div>
                                <label for="data_partida" class="block text-sm font-medium text-gray-700 mb-1">Data de Partida</label>
                                <input type="date" name="data_partida" id="data_partida" value="{{ old('data_partida', $pedido->data_partida) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                            
                            <div>
                                <label for="hora_partida" class="block text-sm font-medium text-gray-700 mb-1">Hora de Partida</label>
                                <input type="time" name="hora_partida" id="hora_partida" value="{{ old('hora_partida', $pedido->hora_partida) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                        </div>
                        @endrole
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="nome_agente" id="nome_agente" value="{{ old('nome_agente', $pedido->nome_agente) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                            
                            <div>
                                <label for="contato_agente" class="block text-sm font-medium text-gray-700 mb-1">Contato do Agente *</label>
                                <input type="text" name="contato_agente" id="contato_agente" value="{{ old('contato_agente', $pedido->contato_agente) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="numero_tripulantes" id="numero_tripulantes" value="{{ old('numero_tripulantes', $pedido->numero_tripulantes) }}" min="0"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            </div>
                            
                            <div>
                                <label for="numero_passageiros" class="block text-sm font-medium text-gray-700 mb-1">Número de Passageiros</label>
                                <input type="number" name="numero_passageiros" id="numero_passageiros" value="{{ old('numero_passageiros', $pedido->numero_passageiros) }}" min="0"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div>
                            ...
                            <textarea name="observacoes" id="observacoes" rows="4" 
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('observacoes', $pedido->observacoes) }}</textarea>
                        </div>
                    </div>

                    <!-- Documentos Existentes e Upload -->
                    <div class="p-6 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                        ...
                        @foreach($documentTypes as $field => $label)
                            @if($pedido->$field && count($pedido->$field) > 0)
                                <div class="mb-6">
                                    <h4 class="text-md font-medium text-gray-800 mb-2">{{ $label }} - Arquivos Existentes</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach($documentTypes as $field => $label)
                                <div>
                                    <label for="{{ $field }}" class="block text-sm font-medium text-gray-700 mb-2">{{ $label }} - Adicionar Novos</label>
                                    <input type="file" name="{{ $field }}[]" id="{{ $field }}" multiple accept=".pdf,.jpg,.jpeg,.png"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
