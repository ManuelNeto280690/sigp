    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 text-xs mt-1">Cadastre um guindaste vinculado a um terminal</p>
            </div>
            <div class="flex items-center space-x-3">
                <a href="{{ route('guindastes.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-1.5 px-3 rounded-lg transition duration-200 flex items-center text-xs">
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('guindastes.store') }}" class="p-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nome" id="nome" value="{{ old('nome') }}" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:border-transparent"
                                   style="focus:ring-color: #3b82f6;" placeholder="Ex: Guindaste 1">
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="tipo" id="tipo" required
                                ...
                                @foreach(['container','graneis_solidos','graneis_liquidos','carga_geral','passageiros','tanque','ro_ro','frigorifico'] as $t)
                                    <option value="{{ $t }}" {{ old('tipo') == $t ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$t)) }}</option>
                                @endforeach
                            </select>
                        </div>
